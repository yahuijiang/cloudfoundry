  ...
  module Dea
    module Client
      ...
      class << self
        ...
        #将find droplet 发 送 到 message_bus
        ...
        def find_all_instances(app)
          num_instances = app.instances#app的instance数量
          all_instances = {}

          flapping_indices = health_manager_client.find_flapping_indices(app)##crash count 大于阈值的instance index
        ...
        #将stop dea instance 的消息“dea.stop”发送到message_bus中
        ...
        def update_uris(app)
          ...
          message = dea_update_message(app)# 生成更新message:  droplet 和uri和版本号
          dea_publish_update(message) #发送dea 的update消息
          app.routes_changed = false
        end
        ...
        # 将find droplet 的消息发送到message_bus上，先找到相应的droplet，也就是特定的instance，然后返回该url
        ...
        #条件：状态为：starting，runing，creashed
        #      路径为：path
          search_options = {
              states: [:STARTING, :RUNNING, :CRASHED],
              path: path,
          }.merge(options)
        #将find droplet的消息发送到message_bus
        ...
        def dea_publish_update(args)#发送dea update 消息
