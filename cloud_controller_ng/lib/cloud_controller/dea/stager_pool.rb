    ...
    class StagerPool
      ...
      def find_stager(stack, memory, disk)
        mutex.synchronize do
          ...
          prune_stale_advertisements#删除过时的advertisement
          best_ad = top_5_stagers_for(memory, disk, stack).sample#从五个符合要求的可用内存最大的stagers中随机选择一个stager
          best_ad && best_ad.stager_id
        end
      end

      # 根据dea_id找到相应的stager，然后将此stager的advertisements的available memory 减小
      ...
      def top_5_stagers_for(memory, disk, stack)
        ...
        end.sort do |advertisement_a, advertisement_b|#按内存从小到大排序
          advertisement_a.available_memory <=> advertisement_b.available_memory
        end.last(5)#选择出可用内存最大的五个stagers
      end
      #删除过时的advertisements
