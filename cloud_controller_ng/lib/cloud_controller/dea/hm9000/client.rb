  ...
  module Dea
    module HM9000
      class Client
        ...
        #返回crash_count 大于阈值的instance index
        def find_flapping_indices(app)
          response = app_state_request(app)#提交对重要app status的请求，return a hash
          return [] unless response

          response['crash_counts'].each_with_object([]) do |crash_count, result|#返回crash_count 大于阈值的instance index
        ...
        private
      #通过http client post app的json 格式的message（guid,version），response 为？？？？
      ...
      #进行提交app status的请求到'/bulk_app_state',返回app 的bulk state
        ...
        def make_request(message)
          ...
          responses = JSON.parse(response.body)#转化成hash
