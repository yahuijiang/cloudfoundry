...
module VCAP::CloudController
  class AppsV3Controller < RestController::BaseController
    ...
    #控制app的create、update、start、stop，展示app等
    ...
    def list
      ...
      facets = params.slice('guids', 'space_guids', 'organization_guids', 'names')#搜索database中app表的条件

      if membership.admin?#如果是admin则获取所有的dataset
        paginated_apps = AppListFetcher.new.fetch_all(pagination_options, facets)
      else#如果是非admin，只获取相应的dataset
    ...
    def create
      ...
      request = parse_and_validate_json(body)#return a hash
      message = AppCreateMessage.create_from_http_request(request)#通过request（a hash object）创建一个message
    ...
    def update(guid)
      ...
      unprocessable!(buildpack_validator.errors.full_messages) unless buildpack_validator.valid?
      #对request的action，
      app = AppUpdate.new(current_user, current_user_email).update(app, message)#update a app
    ...
    def delete(guid)
      ...
      #销毁app:1.销毁blobstroe中的package 2.销毁blobstore中的droplet，3.销毁相应的进程
    ...
    def start(guid)
      ...
      AppStart.new(current_user, current_user_email).start(app)#start即更新app 和process的状态
      [HTTP::OK, @app_presenter.present_json(app)]
    ...
    def stop(guid)
      ...
      AppStop.new(current_user, current_user_email).stop(app)#与start相反，将状态该为stop
      [HTTP::OK, @app_presenter.present_json(app)]
    ...
    def assign_current_droplet(app_guid)
      ...
      #将app进行更新，将app的进程进行更新，并且销毁app中与新的profile不匹配的进程
