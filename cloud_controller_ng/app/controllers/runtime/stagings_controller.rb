...
module VCAP::CloudController
  class StagingsController < RestController::BaseController
    ...
    def download_app(guid)
      ...
      blob = package_blobstore.blob(guid)#如果根据该guid可以找到fog storage 中的files（app files），则建立一个blob，否则返回空
      unless blob
        logger.error "could not find package for #{guid}"
        raise ApiError.new_from_details('AppPackageNotFound', guid)
      end
      @blob_sender.send_blob(app.guid, 'AppPackage', blob, self)#调用sinatra::base sent_file
    end
    ...
    #若blobstore 在本地则使用sinatra 进行send_file ,否则进行重定向到现在该app的url
    def download_droplet(guid)
      ...
      if @blobstore.local?
        ...
        url = @blobstore_url_generator.droplet_download_url(app)#返回app的下载url
